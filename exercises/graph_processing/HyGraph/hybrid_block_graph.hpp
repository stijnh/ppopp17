...
#pragma omp parallel num_threads(num_threads)
    ...
    void run_hybrid_static(const P &program, size_t max_iter, double alpha, size_t gpu_num_streams=4, size_t gpu_batch_size=10) {
        return run_hybrid<false, Verbose>(program, max_iter, gpu_num_streams, gpu_batch_size, alpha);
    }

    template <bool Verbose=false>
    void run_hybrid_dynamic(const P &program, size_t max_iter, size_t gpu_num_streams=4, size_t gpu_batch_size=10) {
