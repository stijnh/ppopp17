...
#pragma omp parallel
    {
        ...
        size_t items_per_thread = (num / num_threads) + 1;

        size_t start = min(items_per_thread * (tid + 0), num);
        size_t end   = min(items_per_thread * (tid + 1), num);
        size_t mid;

        sort(first + start, first + end, comp);

#pragma omp barrier

        for (size_t delta = 1; delta < num_threads; delta *= 2) {
            start = min(items_per_thread * delta * (2 * tid + 0), num);
            mid   = min(items_per_thread * delta * (2 * tid + 1), num);
            end   = min(items_per_thread * delta * (2 * tid + 2), num);

            inplace_merge(first + start, first + mid, first + end, comp);
